...
public class VistaInformeController implements Initializable {
    ...
    public void editar(MouseEvent e) {
        ...
        stmodel.loadpagemodificarinforme("registrar", viewp, informe);
    }
    ...
    public void eliminar(MouseEvent e) {
        ...
        btnConfirmar.setOnAction(event -> {
            boolean decision = rgmodel.eliminarDocumentos(informe.getId_documento());
            if (decision) {
    ...
    public void setInforme(InformePasantia informe) {
        ...
        if (informe != null) {
            System.out.println("Objeto recibido: " + informe.getTitulo()); // Depuración
            mostrardatos(informe);
            informe.getdatos();
        } else {
    ...
    public void mostrardatos(InformePasantia inf) {

        textid.setText(String.valueOf(inf.getId_documento()));
        texttitulo.setText(inf.getTitulo());
        textautor.setText(inf.getAutor());
        textcedula.setText(inf.getCedula());
        textcarrera.setText(inf.getCarrera());
        textempresa.setText(inf.getEmpresa());

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        textfecha.setText(inf.getFecha_publicacion().format(formatter));
        try {
            imgportada.setImage(Documento.convertPdfBytesToImage(inf.getImg_portada()));
        } catch (IOException ex) {
            Logger.getLogger(VistaInformeController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void verportada(MouseEvent e) {
        abrirDialogoImagen("Portada", informe.getImg_portada());
    }

    @FXML
    private void verresumen(MouseEvent e) {
        abrirDialogoImagen("Resumen", informe.getResumen());
    }
...
// Cambia por un método más seguro para almacenar contraseñas
    ...
    private void mostrarDialogoCorreo(MouseEvent event) {
        ...
        btnEnviar.setOnAction(e -> {
            ...
            if (esCorreoValido(correo)) {
                ...
                boolean enviado = enviarCorreo(correo, informe, mensaje); // 'tesis' es tu objeto de tipo TrabajoGrado
    ...
    private boolean enviarCorreo(String correoDestinatario, InformePasantia inf, String msjpersonalizado) {
        ...
        try {
            ...
            String contenido = "<html><body>"
                    ...
                    + "<p>Título: " + inf.getTitulo() + "</p>"
                    + "<p>Autor: " + inf.getAutor() + "</p>" + "<p>Nota: " + msjpersonalizado + "</p>"
                    + "</body></html>";
